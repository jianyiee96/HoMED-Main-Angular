...
export class LoginScreenComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let message = this.sessionService.getLogoutMessage()
    this.sessionService.setLogoutMessage("-")

    console.log(message)
    if (message == "DUPLICATE") {
      this.msgs = []
      this.msgs.push({ severity: 'info', summary: '', detail: 'Duplicate login detected. You have been logged out for security reasons.' })
      this.sessionService.setLogoutMessage("-")
    }
  }
